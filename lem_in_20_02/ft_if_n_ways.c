...
int			ft_count_steps(t_map *map, t_map *sh_w)
{
	int res;
	int *way;
	t_map *tmp;
	...
	tmp = ft_copy_map(map);
	res = 0;
	while (sh_w->room[map->start].links)
	{
		way = ft_bfs(sh_w, sh_w->room[map->start], sh_w->room[map->end], 0);
		res += way[0];
		ft_move_sh_w(sh_w, tmp, way);
		free(way);
	}
	ret_1_2(sh_w, tmp);
	ft_exit(tmp, NULL, 1);
